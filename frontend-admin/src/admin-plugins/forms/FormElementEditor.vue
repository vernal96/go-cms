...
const props = defineProps<{ disabled: boolean; initialType: ElementType; element?: FormElement | null; availableTypes: ElementTypeMetadata[]; accessToken: string; permissions: ReadonlySet<string> }>()
...
function reset(): void {
  ...
  Object.assign(state, {
    code: item?.code ?? '', type: item?.type ?? props.initialType, text: String(config.text ?? ''), content: String(config.content ?? ''),
...
function payload(): Pick<FormElement, 'code' | 'type' | 'config'> { return { code: state.code.trim(), type: state.type, config: config() } }

...
### Проверки границ расширения

- HTTP-маршруты профиля не могут объявлять `/api`, `/_cms` и их дочерние
  пути: эти пространства принадлежат платформе. Для административного API
  сайта используйте `httptransport.SiteManagementProvider`.
- Источник миграций каждого модуля на одном соединении должен иметь отдельную
  SQL-схему: два источника не могут использовать общую таблицу истории.
- `RegistryForConfig` имеет приоритет над `Registry` и для blueprint, и для
  общего каталога прав. Каталог объединяет права всех профилей.
- Все пункты меню сайта, включая скрытые по правам, проверяются до сохранения
  и публикации кандидата runtime. Коллизия с глобальным меню отменяет операцию.

...
func TestPostgresMigrationsAndSiteRepository(t *testing.T) {
	host := os.Getenv("CMS_TEST_PROJECT_POSTGRES_HOST")
	if host == "" {
		t.Skip("set CMS_TEST_PROJECT_POSTGRES_HOST to run the PostgreSQL integration test")
	}

	port := 5432
	if value := os.Getenv("CMS_TEST_PROJECT_POSTGRES_PORT"); value != "" {
		parsed, err := strconv.Atoi(value)
		if err != nil {
			t.Fatalf("parse CMS_TEST_PROJECT_POSTGRES_PORT: %v", err)
		}
		port = parsed
	}

	sslMode := os.Getenv("CMS_TEST_PROJECT_POSTGRES_SSL_MODE")
	if sslMode == "" {
		sslMode = "disable"
	}
	...
	connector, err := connectorpostgres.New(ctx, connectorpostgres.Config{
		...
		Host:            host,
		Port:            port,
		Database:        os.Getenv("CMS_TEST_PROJECT_POSTGRES_DB"),
		User:            os.Getenv("CMS_TEST_PROJECT_POSTGRES_USER"),
		Password:        os.Getenv("CMS_TEST_PROJECT_POSTGRES_PASSWORD"),
		SSLMode:         sslMode,
		MaxConns:        4,
		MinConns:        0,

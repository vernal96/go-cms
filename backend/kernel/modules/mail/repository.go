...
type Repository interface {
	...
	ListMessages(context.Context, site.ID, PageQuery) (MessagePage, error)
	MessageDetail(context.Context, site.ID, MessageID) (MessageDetail, error)
	DeleteMessage(context.Context, site.ID, MessageID) error
	ClaimMessage(context.Context, site.ID, MessageID) (Message, DeliveryAttempt, bool, error)
	FinishAttempt(context.Context, MessageID, int, DeliveryResult, error) error

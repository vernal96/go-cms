...
CREATE TEMP TABLE resource_field_staging_preflight
(
    ready BOOLEAN NOT NULL,
    CONSTRAINT resource_field_staging_incomplete_or_stale_run_prepare CHECK (ready)
);
INSERT INTO resource_field_staging_preflight (ready)
SELECT NOT EXISTS (
    SELECT 1
    FROM core.resources AS resource
    LEFT JOIN core.resource_field_migration_manifest AS manifest
      ON manifest.resource_id = resource.id AND manifest.site_id = resource.site_id
    WHERE manifest.resource_id IS NULL OR manifest.source_digest <> md5(resource.settings::text)
);
TRUNCATE resource_field_staging_preflight;
ALTER TABLE resource_field_staging_preflight
    DROP CONSTRAINT resource_field_staging_incomplete_or_stale_run_prepare,
    ADD CONSTRAINT resource_field_staging_rows_require_manifest CHECK (ready);
INSERT INTO resource_field_staging_preflight (ready)
SELECT NOT EXISTS (
    SELECT 1
    FROM core.resource_field_migration_rows AS value
    LEFT JOIN core.resource_field_migration_manifest AS manifest
      ON manifest.resource_id = value.resource_id AND manifest.site_id = value.site_id
    WHERE manifest.resource_id IS NULL
);
DROP TABLE resource_field_staging_preflight;
...
SELECT
    ...
    value_string, value_integer, value_float, value_boolean,
    value_timestamp, value_reference, value_json
FROM core.resource_field_migration_rows;

ALTER TABLE core.resources DROP COLUMN settings;
DROP TABLE core.resource_field_migration_rows;
DROP TABLE core.resource_field_migration_manifest;

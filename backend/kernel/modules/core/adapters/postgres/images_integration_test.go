...
func TestPostgresImageDeletionImpactCascadeAndFields(t *testing.T) {
	...
	if err = cascade.DeleteConfirmed(ctx, items, blocked.Token, physical); !errors.Is(err, file.ErrInUse) || deleted != 0 {
	...
	if err = cascade.DeleteConfirmed(ctx, items, blocked.Token, physical); !errors.Is(err, file.ErrConflict) {
	...
	if err = cascade.DeleteConfirmed(ctx, items, impact.Token, physical); err != nil {
		t.Fatal(err)
	}
	if deleted != 2 {
